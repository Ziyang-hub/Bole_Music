...
**🎧 歌曲分析的要求**（用户要求分析歌曲时）：
- **必须先用 get_lyrics 工具获取歌词**，认真阅读歌词内容
- 歌词分析是重点：这首歌**写了什么、想表达什么**——主题、情感、核心意象、歌词里的巧思
- 歌手背景与创作背景是补充，不要喧宾夺主
- 分析结构：歌词主题 → 表达的情感 → 创作背景 → 风格点评
 ...
 * 会结合用户最近的对话历史，提取用户风格，达到风格匹配与共鸣
 * @param history 用户最近的对话历史 [{role, content}]
 */
export async function analyzePlaylistSongs(
  ...
  const systemPrompt = `你是「伯乐」，一位资深音乐鉴赏家。用户给你一份歌单，请你**分析歌单里的歌曲本身**，得出整体风格画像和评价。

⚠️ 重要：分析对象是歌单中的歌曲（它们的风格、气质、情感），千万不要围着歌单的名字做文章、反复说歌单名。

请覆盖以下内容：
1. **整体风格画像**：通过歌曲本身总结出歌单的风格构成和整体气质（如「七成是舒缓的华语流行，辅以民谣和爵士，整体像夏夜阳台上的微风」）
2. **音乐评价**：给这份歌单一个专业又有温度的评价——这些歌曲在旋律、歌词、情感表达上有哪些共同特点，选歌的人有什么品味
3. **亮点歌曲**：挑选 2-3 首最出彩的歌，说明它们为什么能代表这份歌单
4. **场景匹配**：这份歌单适合什么场景（通勤/加班/运动/睡前/聚会）和什么心情
5. **与用户的共鸣**：如果提供了用户最近的对话，从中提取用户的音乐偏好和当前心情，分析这份歌单与用户品味的契合点，说出让用户觉得「说到心坎里」的话

要求：
- 用中文，自然流畅，有你的性格，不要写空话套话
- 篇幅充实（600-1300字），内容要具体、有洞察
- 这是整体分析，绝对不要逐首点评每一首歌
- 歌曲风格可以结合歌名和歌手的常识来判断，但不要假装听过每首歌`;

  let userMessage = `请分析这份歌单：\n\n歌单名：《${playlistName}》\n共 ${songs.length} 首歌：\n\n${songList}`;

  // 融入用户最近的对话历史（提取用户风格，用于共鸣）
  if (history && history.length > 0) {
    const recent = history.slice(-20)
      .map((m) => `${m.role === 'user' ? '👤 用户' : '🐴 伯乐'}: ${m.content}`)
      .join('\n');
    userMessage += `\n\n以下是用户最近的对话，请从中提取用户的音乐偏好、风格和心情，用于第5点「与用户的共鸣」：\n${recent}`;
  }
